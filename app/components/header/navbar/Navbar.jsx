    ...
    <nav className={`fixed ${isTopBannerVisible ? 'top-0' : 'top-0'} w-full z-40 transition-all duration-300 px-[10px] md:px-[20px] lg:px-[20px] xl:px-[40px] py-[5px] ${navbarBg} navbar`}>
      <div className="flex justify-between items-center w-full">
        ...
        <div className="flex-grow hidden xl:flex justify-center space-x-6 md:space-x-5 lg:space-x-5">
          ...
          <Link href="/about" className="text-white font-jura text-[14px] lg:text-[18px] font-black hover:text-[#ff0a01]" onClick={closeMenu}>
            ABOUT US
          </Link>
          <Link href="/services" className="text-white text-[14px] lg:text-[18px] font-black font-jura hover:text-[#ff0a01]" onClick={closeMenu}>
            SERVICES
          </Link>
          {/* <Link href="/" className="text-white font-jura text-[14px] lg:text-[18px] font-black hover:text-[#ff0a01]" onClick={closeMenu}>
            TESTIMONIAL
          </Link> */}
          <Link href="/contact" className="text-white font-jura text-[14px] lg:text-[18px] font-black hover:text-[#ff0a01]" onClick={closeMenu}>
           CONTACT US
          </Link>

        </div>
        <div className="hidden xl:flex items-center w-[145px] md:w-[192px] lg:w-[192px] h-[44px]">
            <Link href="/contact" className="w-[200px] cursor-pointer px-2 py-[7px] lg:py-[13px] md:py-[13px] font-jura font-normal md:font-bold bg-gradient-to-r from-[#F56545] to-[#ff0a01] text-white ml-2 transition duration-300 flex items-center justify-center relative overflow-hidden rounded-full ">
      ...
      {menuOpen && (
        <div ref={menuRef} className="xl:hidden bg-[#ff0a01] w-[50%] absolute right-0 top-full mx-4">
          <div className="flex flex-col items-start px-4 py-4">
            ...
            <Link href="/about" className="block w-full text-left px-4 py-4 text-white font-jura text-[14px] hover:text-[#ff0a01]" onClick={closeMenu}>
              ABOUT US
            </Link>
            <Link href="/services" className="block w-full text-left px-4 py-4 text-white font-jura font-bold text-[14px] hover:text-[#ff0a01]" onClick={closeMenu}>
              SERVICES
            </Link>
            ...
            <Link href="/contact" className="block w-full text-left px-4 py-4 text-white font-jura text-[14px] font-bold hover:text-[#ff0a01]" onClick={closeMenu}>
              CONTACT US
            </Link>
